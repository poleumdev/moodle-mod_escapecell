...
$PAGE->set_title('Escape Cell Level 2 (factice)');
$PAGE->set_heading('Escape Cell Level 2 (factice)');
...
echo '<form id="ScoreForm" name="ScoreForm" action="../../score.php">
  ...
  <input type="text" value="2" id="level" name="level" />';
echo '<input type="text" value="' . $cmid .'" id="cmid" name="cmid"/>';
echo '</form>';
echo '<button onclick="abandon()">Abandoner</button>';
echo '<button onclick="passer()">Passer</button>';

echo '<script type="text/javascript">
function abandon() {
    document.ScoreForm.score.value = "-1";
